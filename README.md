...
- An optional second text-only Qwen3-VL pass expands factual image records into a validated 250-500 word direction without paying image-token cost twice.
...
The Director separates output format from image understanding. **Keep my prompt** preserves the wording, **Clear one-line instruction** creates heading-free prose, and **Structured production brief** builds the four-section format. **Analyze image pixels** makes the Loader's full Qwen3-VL analyzer inspect the real references, assign roles/retention, and fill visible factual descriptions. **Detailed second pass** then reuses that model text-only by default, or uses a separately selected full Qwen3-VL 8B checkpoint, to write and validate a 250-500 word direction. Both caches ignore seed-only changes.
...
Image analysis uses ComfyUI's native full Qwen3-VL implementation. The maintained workflow selects `qwen3vl_4b_fp8_scaled.safetensors` automatically when present, downscales analyzer-only copies to 512 px by default, and leaves H3's inputs untouched. The prompt writer defaults to **Same as image analyzer**, so the second pass requires no extra model. A full 8B file from [Comfy-Org/Qwen3-VL](https://huggingface.co/Comfy-Org/Qwen3-VL) is an optional writing-quality upgrade, not an automatic download. Manual card descriptions are never overwritten.
...
Expected model categories and sources:

```text
ComfyUI/models/diffusion_models/  FL2VA + REF2VA: https://huggingface.co/Comfy-Org/MiniMax-H3
ComfyUI/models/text_encoders/     32B H3 conditioning encoder: https://huggingface.co/Comfy-Org/MiniMax-H3
ComfyUI/models/text_encoders/     full 4B/8B analyzer-writer: https://huggingface.co/Comfy-Org/Qwen3-VL
ComfyUI/models/vae/               H3 video VAE: https://huggingface.co/Comfy-Org/MiniMax-H3
ComfyUI/models/vae_approx/        TAEH3 preview: https://huggingface.co/Kijai/MiniMax-H3-TAE
ComfyUI/models/loras/             LightX: https://huggingface.co/Kijai/MiniMax-H3_comfy
ComfyUI/models/loras + pdd_heads/ PDD: https://huggingface.co/Mamad8/MiniMaxH3_R2V-PDD-Turbo-LoRA-Mamad8
```
...
The bundled workflow includes an opt-in **A/B Matrix** for diagnosing whether quality loss comes from resolution or acceleration. It requests `0.40`, `1.00`, and `2.00 MP`. **Same seed for all** is the fair default. **New seed each row** gives each resolution a different seed while keeping its Base/LoRA pair comparable. **New seed every image** is a diversity sweep and is labeled as such because it no longer isolates the accelerator.

The node returns one labeled two-column grid. Each cell reports its seed, requested megapixels, actual aligned dimensions and megapixels, profile/LoRA status, and CUDA-synchronized time spent inside `SamplerCustomAdvanced`. Conditioning, adapter setup, VAE decode, grid composition, and queue overhead are deliberately excluded from the sampling figure. The first sampled cell can still include lazy model initialization, while later cells may benefit from warm caches, so treat the labels as honest sampler-call timings rather than a laboratory benchmark. The detailed string report exposes the same measurements for copying into an issue or experiment log.

The matrix is disabled by default because enabling it performs six complete generations. When enabled, its context output blocks the ordinary sampling branch, preventing the accidental seventh generation performed by older workflow revisions. H3's native area cap can make the `1.00 MP` and `2.00 MP` rows resolve to the same actual dimensions; this is intentional and makes a misleading target-size assumption immediately visible. If the Director currently uses a Base profile, choose LightX or PDD explicitly in the matrix's accelerator control.
...
5. A disabled-by-default six-run seed-aware A/B quality matrix that gates the normal branch.
...
- **H3 Studio · A/B Matrix** — six opt-in generations comparing `0.40/1.00/2.00 MP` with and without the selected accelerator, with fixed, row-paired, or per-image seeds in one timed labeled grid.
...
State is versioned (`schema_version: 8`) and migrates older workflows forward. Schema 8 adds the optional cached text-only prompt-director pass without enabling it in old saved graphs; the maintained bundled workflow enables it explicitly. Workflows store the full Studio state plus compatibility widget values, so the node remains inspectable even if the frontend extension is temporarily unavailable.
