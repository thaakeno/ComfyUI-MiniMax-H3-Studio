/*
 * H3 Studio media/mention editor.
 *
...
const TEXT = {
    ...
    mainTitle: "MiniMax H3 Studio · Image Director",
    loaderTitle: ZH_BROWSER ? "MiniMax H3 Studio \u52a0\u8f7d\u5668" : "MiniMax H3 Studio Loader",
    outputTitle: ZH_BROWSER ? "MiniMax H3 Studio \u8f93\u51fa" : "MiniMax H3 Studio Output",
    category: "H3 Studio",
...
const OPTION_DEFS = {
    mode: {
        [MODE_IMAGE]: "Auto · T2I / I2I",
        [MODE_REFERENCE]: "Reference Edit · REF2VA",
    },
    keyframe_role: {
...
function convertNativeMediaConnection(targetNode, inputIndex, linkInfo = null) {
    if (!isTarget(targetNode) || targetNode.__h3sVirtualWireClearing) return false;
    ...
    targetNode.__h3sVirtualWireClearing = true;
    try {
        ...
        targetNode.__h3sVirtualWireClearing = false;
    }
...
function setNativeSearchVisualSuppression(enabled) {
    const className = "minimax-h3s-easy-hide-native-search";
    if (enabled) {
...
function installQuickCreateCapture(canvas) {
    ...
    if (canvas === quickCreateCaptureCanvas && canvas.__h3sEasyQuickCreateCaptureInstalled) return true;
    ...
    canvas.__h3sEasyQuickCreateCaptureInstalled = true;
    const handler = (event) => {
    ...
    quickCreateCaptureCleanup = () => {
        ...
        canvas.__h3sEasyQuickCreateCaptureInstalled = false;
...
function patchCanvas() {
    ...
    if (!canvas || canvas.__h3sEasyCanvasPatched || typeof canvas.drawConnections !== "function") return;
    canvas.__h3sEasyCanvasPatched = true;
    patchedCanvas = true;
...
function editorText(editor) {
    ...
    const visit = (node) => {
        ...
        if (node.classList?.contains("h3s-mention-chip")) {
...
function refreshMentionPreviews() {
    for (const node of app.graph?._nodes || []) {
        ...
        for (const chip of node.__h3sEditor?.querySelectorAll?.(".h3s-mention-chip") || []) {
        ...
        if (node.__h3sEditor) syncPromptFromEditor(node, false);
        const menu = node.__h3sMentionMenu;
        if (menu) {
...
function updateMentionChip(chip, option) {
    ...
    chip.title = option.pending
        ...
        : nextFullLabel;
    const label = chip.querySelector?.(".h3s-mention-chip-label");
    if (label) label.textContent = `@${nextLabel}`;
    const thumb = chip.querySelector?.(".h3s-mention-chip-thumb");
...
function watchMediaSourceNode(node) {
    if (!node) return;
    node.__h3sMediaSourceWatchInstalled = true;
    for (const widget of node.widgets || []) {
        if (!widget || widget.__h3sMediaSourceWatchInstalled) continue;
        widget.__h3sMediaSourceWatchInstalled = true;
...
function makeMentionThumb(option, menu = false) {
    const className = menu ? "h3s-mention-menu-thumb" : "h3s-mention-chip-thumb";
...
function makeMentionChip(option) {
    ...
    chip.className = `h3s-mention-chip${option.pending ? " is-pending" : option.unresolved ? " is-unresolved" : ""}`;
    ...
    label.className = "h3s-mention-chip-label";
    label.textContent = `@${option.label || ""}`;
    chip.append(makeMentionThumb(option), label);
    chip.addEventListener("pointerdown", (event) => {
        if (event.target?.closest?.(".h3s-mention-chip-label")) return;
        event.preventDefault();
        event.stopPropagation();
...
function serializeEditorDoc(editor) {
    ...
    const visit = (item) => {
        ...
        if (item.classList?.contains("h3s-mention-chip")) {
            parts.push({
                type: "mention",
...
function syncPromptFromEditor(node, markDirty = true) {
    const editor = node?.__h3sEditor;
    const widget = getWidget(node, "prompt");
    if (!editor || !widget || node.__h3sEditorSyncing) return;
    node.__h3sEditorSyncing = true;
    try {
...
function editorPromptNode(editor) {
    return editor?.__h3sPromptNode || null;
}

function editorFromEvent(event) {
    const target = event?.target;
    if (target?.closest) {
        const editor = target.closest(".h3s-prompt-editor");
        if (editor) return editor;
    }
    const active = typeof document !== "undefined" ? document.activeElement : null;
    return active?.closest?.(".h3s-prompt-editor") || activePromptNode?.__h3sEditor || null;
}
...
function ensurePromptHistory(node) {
    ...
    if (node.__h3sPromptHistory) return node.__h3sPromptHistory;
    const doc = clonePromptDoc(serializeEditorDoc(editor));
    node.__h3sPromptHistory = {
        undo: [{ doc }],
        redo: [],
        lastKey: promptDocKey(doc),
        applying: false,
    };
    return node.__h3sPromptHistory;
}

function resetPromptHistory(node) {
    node.__h3sPromptHistory = null;
...
function applyPromptHistoryEntry(node, entry) {
    const history = node?.__h3sPromptHistory;
    const editor = node?.__h3sEditor;
    const widget = getWidget(node, "prompt");
...
function handlePromptUndoRedoCapture(event) {
    ...
    if (node && !node.__h3sPromptComposing) {
...
function handlePromptHistoryBeforeInputCapture(event) {
    ...
    const node = editorPromptNode(editorFromEvent(event));
    if (!node || node.__h3sPromptComposing) return;
...
function ensurePromptUndoRedoShield() {
    ...
    window.addEventListener("pointerdown", (event) => {
        const editor = event?.target?.closest?.(".h3s-prompt-editor");
...
function preparePromptEditorForUndo(editor) {
    if (!editor) return;
    if (editor.__h3sUndoPrepared && editor.dataset?.h3UndoVersion === PROMPT_UNDO_VERSION) return;
    editor.setAttribute("data-h3s-undo-version", PROMPT_UNDO_VERSION);
    try {
...
function getMentionRange(editor) {
    ...
    const visit = (node) => {
        if (node.nodeType === Node.TEXT_NODE) {
            if (!node.parentElement?.closest?.(".h3s-mention-chip")) units.push({ kind: "text", node });
            return;
        }
        if (node.nodeType !== Node.ELEMENT_NODE) return;
...
function closeMentionMenu(node) {
    const menu = node?.__h3sMentionMenu;
    menu?.element?.remove?.();
    if (node) node.__h3sMentionMenu = null;
}
...
function chooseMention(node, option) {
    const state = node?.__h3sMentionMenu;
...
function renderMentionMenu(node) {
    ...
    title.className = "h3s-mention-menu-title";
    title.textContent = TEXT.mentionTitle;
    element.append(title);
    if (!options.length) {
        const empty = document.createElement("div");
        empty.className = "h3s-mention-menu-empty";
    ...
    options.forEach((option, index) => {
        ...
        item.className = `h3s-mention-menu-item${index === activeIndex ? " is-active" : ""}`;
        const main = document.createElement("div");
        main.className = "h3s-mention-menu-main";
        main.textContent = option.label;
        main.title = option.fullLabel || option.label || "";
        const detail = document.createElement("div");
        detail.className = "h3s-mention-menu-detail";
        detail.textContent = option.source;
        const text = document.createElement("div");
        text.append(main, detail);
        item.append(makeMentionThumb(option, true), text);
        item.addEventListener("pointermove", () => {
            if (!node.__h3sMentionMenu || node.__h3sMentionMenu.activeIndex === index) return;
            node.__h3sMentionMenu.activeIndex = index;
            renderMentionMenu(node);
        });
...
function openMentionMenu(node, editor) {
    ...
    const existing = node.__h3sMentionMenu;
    if (existing) {
    ...
    element.className = "h3s-mention-menu";
    applyNativeEditorTheme(element);
    document.body.append(element);
    node.__h3sMentionMenu = { element, mention, options, activeIndex: 0 };
    renderMentionMenu(node);
...
function applyNativeEditorTheme(element) {
    ...
    if (element.__h3sNativeThemeSignature === signature) return;
    element.__h3sNativeThemeSignature = signature;
    element.classList?.toggle("h3s-native-vue-nodes", modern);
    element.classList?.toggle("h3s-native-legacy-nodes", !modern);
    if (modern) {
        element.style.setProperty("--h3s-native-widget-bg", "var(--component-node-widget-background, var(--secondary-background, #222))");
        element.style.setProperty("--h3s-native-widget-text", "var(--component-node-foreground, var(--base-foreground, #ddd))");
        element.style.setProperty("--h3s-native-widget-outline", "var(--component-node-widget-background-highlighted, var(--border-default, rgba(255, 255, 255, 0.18)))");
        element.style.setProperty("--h3s-native-widget-focus", "var(--component-node-widget-background-highlighted, var(--border-default, rgba(255, 255, 255, 0.28)))");
        element.style.setProperty("--h3s-native-widget-muted", "var(--component-node-foreground-secondary, var(--muted-foreground, rgba(255, 255, 255, 0.42)))");
        element.style.setProperty("--h3s-native-menu-bg", "var(--component-node-widget-background, var(--comfy-menu-bg, #1f1f1f))");
        element.style.setProperty("--h3s-native-widget-radius", "var(--radius-lg, 8px)");
        element.style.setProperty("--h3s-native-widget-padding", "8px 12px");
        element.style.setProperty("--h3s-native-widget-line-height", "var(--text-xs--line-height, 1.3333333)");
        element.style.setProperty("--h3s-native-widget-text-size", "var(--text-xs, var(--comfy-textarea-font-size, 12px))");
        return;
    }
    element.style.setProperty("--h3s-native-widget-bg", `var(--comfy-input-bg, ${widgetBg})`);
    element.style.setProperty("--h3s-native-widget-text", `var(--input-text, ${widgetText})`);
    element.style.setProperty("--h3s-native-widget-outline", `var(--border-color, ${outline})`);
    element.style.setProperty("--h3s-native-widget-focus", `var(--border-color, ${outline})`);
    element.style.setProperty("--h3s-native-widget-muted", "rgba(255, 255, 255, 0.42)");
    element.style.setProperty("--h3s-native-menu-bg", `var(--comfy-menu-bg, ${menuBg})`);
    element.style.setProperty("--h3s-native-widget-radius", "0px");
    element.style.setProperty("--h3s-native-widget-padding", "2px");
    element.style.setProperty("--h3s-native-widget-line-height", "normal");
    element.style.setProperty("--h3s-native-widget-text-size", "var(--comfy-textarea-font-size, 12px)");
}
...
function syncEditorThemes(force = false) {
    ...
    for (const node of app.graph?._nodes || []) {
        if (!isTarget(node)) continue;
        applyNativeEditorTheme(node.__h3sEditorWrap);
        applyNativeEditorTheme(node.__h3sMentionMenu?.element);
    }
    app.graph?.setDirtyCanvas?.(true, true);
}
...
function patchEditorKeyHandling() {
    const proto = globalThis.LGraphCanvas?.prototype;
    if (!proto || proto.__h3sPromptKeyHandlingPatched || typeof proto.processKey !== "function") return;
    proto.__h3sPromptKeyHandlingPatched = true;
    const original = proto.processKey;
    proto.processKey = function processKeyWithH3PromptEditor(event) {
        const editor = event?.target?.closest?.(".h3s-prompt-editor")
            || document.activeElement?.closest?.(".h3s-prompt-editor")
            || activePromptNode?.__h3sEditor;
        if (editor) {
...
function hideOriginalPromptWidget(widget) {
    if (!widget) return;
    if (!widget.__h3sPromptHidden) {
        widget.__h3sPromptHidden = true;
        widget.__h3sOriginalType = widget.type;
        widget.__h3sOriginalComputeSize = widget.computeSize;
        widget.__h3sOriginalHidden = widget.hidden;
        widget.__h3sOriginalOptionsHidden = widget.options?.hidden;
        widget.__h3sOriginalOptionsCanvasOnly = widget.options?.canvasOnly;
    }
    widget.hidden = true;
...
function restoreOriginalPromptWidget(widget) {
    if (!widget?.__h3sPromptHidden) return;
    widget.type = widget.__h3sOriginalType || "customtext";
    widget.computeSize = widget.__h3sOriginalComputeSize || (() => [220, 120]);
    widget.hidden = widget.__h3sOriginalHidden ?? false;
    setWidgetOption(widget, "hidden", widget.__h3sOriginalOptionsHidden);
    setWidgetOption(widget, "canvasOnly", widget.__h3sOriginalOptionsCanvasOnly);
    widget.__h3sPromptHidden = false;
}

function hideDomEditorWidget(widget) {
    if (!widget) return;
    if (!widget.__h3sEditorHidden) {
        widget.__h3sEditorHidden = true;
        widget.__h3sEditorType = widget.type;
        widget.__h3sEditorComputeSize = widget.computeSize;
    }
    widget.hidden = true;
    setWidgetOption(widget, "hidden", true);
    widget.type = "hidden";
...
function showDomEditorWidget(widget) {
    if (!widget?.__h3sEditorHidden) return;
    widget.type = widget.__h3sEditorType || "h3_prompt_mentions";
    widget.computeSize = widget.__h3sEditorComputeSize || (() => [220, 96]);
    widget.hidden = false;
    setWidgetOption(widget, "hidden", false);
    widget.__h3sEditorHidden = false;
}
...
function hideConditionalWidget(widget) {
    ...
    if (widget.type !== "hidden" && widget.type !== "converted-widget") widget.__h3sConditionalOrigType = widget.type;
    if (!Object.prototype.hasOwnProperty.call(widget, "__h3sConditionalOrigComputeSize")) {
        widget.__h3sConditionalOrigComputeSize = originalComputeSize;
        widget.__h3sConditionalHadComputeSize = Object.prototype.hasOwnProperty.call(widget, "computeSize");
        widget.__h3sConditionalOrigHidden = originalHidden;
        widget.__h3sConditionalOrigOptionsHidden = widget.options?.hidden;
        widget.__h3sConditionalOrigOptionsCanvasOnly = widget.options?.canvasOnly;
        widget.__h3sConditionalOrigComputedHeight = widget.computedHeight;
        widget.__h3sConditionalHadComputedHeight = Object.prototype.hasOwnProperty.call(widget, "computedHeight");
    }
    widget.hidden = true;
...
function showConditionalWidget(widget) {
    ...
    widget.hidden = widget.__h3sConditionalOrigHidden ?? false;
    if (widget.inputEl) widget.inputEl.style.display = "";
    if (widget.element) widget.element.style.display = "";
    if (widget.type === "hidden") widget.type = widget.__h3sConditionalOrigType || "combo";
    if (widget.__h3sConditionalHadComputeSize) widget.computeSize = widget.__h3sConditionalOrigComputeSize;
    else delete widget.computeSize;
    if (widget.__h3sConditionalHadComputedHeight) widget.computedHeight = widget.__h3sConditionalOrigComputedHeight;
    ...
    if (widget._state) {
        ...
        if (widget.__h3sConditionalHadComputedHeight) widget._state.computedHeight = widget.computedHeight;
...
function syncEditorMode(node) {
    ...
    const widget = getWidget(node, "prompt");
    const editor = node.__h3sEditor;
    const wrap = node.__h3sEditorWrap;
    const domWidget = node.__h3sDomWidget;
...
function isMentionChip(node) {
    return node?.nodeType === Node.ELEMENT_NODE && node.classList?.contains("h3s-mention-chip");
}
...
function deleteChipNearCaret(editor, node, direction) {
    ...
    const directChip = editorNode.nodeType === Node.ELEMENT_NODE
        ? editorNode.closest?.(".h3s-mention-chip")
        : editorNode.parentElement?.closest?.(".h3s-mention-chip");
...
function ensurePromptEditor(node) {
    if (node.__h3sEditor) {
        preparePromptEditorForUndo(node.__h3sEditor);
        return;
    }
    ...
    wrap.className = "h3s-prompt-editor-wrap";
    wrap.style.minHeight = "0px";
    applyNativeEditorTheme(wrap);
    const editor = document.createElement("div");
    editor.className = "comfy-multiline-input h3s-prompt-editor";
    editor.contentEditable = "true";
    preparePromptEditorForUndo(editor);
    editor.__h3sPromptNode = node;
    editor.tabIndex = 0;
    ...
    editor.addEventListener("beforeinput", (event) => {
        if (node.__h3sDialogueHashHandled) {
            node.__h3sDialogueHashHandled = false;
            event.preventDefault();
            event.stopPropagation();
            event.stopImmediatePropagation?.();
            return;
    ...
    editor.addEventListener("input", (event) => {
        syncPromptFromEditor(node);
        if (event?.isComposing || event?.inputType === "insertCompositionText" || node.__h3sPromptComposing) {
    ...
    editor.addEventListener("compositionstart", () => {
        node.__h3sPromptComposing = true;
    });
    editor.addEventListener("compositionend", () => {
        node.__h3sPromptComposing = false;
        syncPromptFromEditor(node);
        pushPromptHistory(node);
    });
    ...
    editor.addEventListener("keydown", (event) => {
        ...
        if (event.key === "#" && !event.ctrlKey && !event.metaKey && !event.altKey && insertDialogueBlockAtSelection(node, editor)) {
            event.preventDefault();
            event.stopPropagation();
            node.__h3sDialogueHashHandled = true;
            setTimeout(() => { node.__h3sDialogueHashHandled = false; }, 0);
            syncPromptFromEditor(node);
            pushPromptHistory(node);
            return;
        }
    ...
    editor.addEventListener("blur", () => {
        syncPromptFromEditor(node);
        setTimeout(() => {
            if (!node.__h3sMentionMenu?.element?.matches?.(":hover")) closeMentionMenu(node);
        }, 160);
    });
    wrap.addEventListener("pointerdown", (event) => {
        event.stopPropagation();
        if (!event.target?.closest?.(".h3s-mention-chip")) closeMentionMenu(node);
    });
    ...
    wrap.append(editor);
    node.__h3sEditor = editor;
    node.__h3sEditorWrap = wrap;
    ...
    if (!domWidget) {
        ...
        wrap.remove();
        node.__h3sEditor = null;
        node.__h3sEditorWrap = null;
        return;
    }
    node.__h3sDomWidget = domWidget;
...
function installPromptEditorSoon(node) {
    if (!node || node.__h3sPromptInstallPending || node.__h3sPromptInstallRetry || node.__h3sEditor) return;
    const now = typeof performance !== "undefined" ? performance.now() : Date.now();
    if (now < (Number(node.__h3sPromptInstallNextAt) || 0)) return;
    node.__h3sPromptInstallPending = true;
    const run = () => {
        node.__h3sPromptInstallPending = false;
        ensurePromptEditor(node);
        if (node.__h3sEditor) {
            node.__h3sPromptInstallAttempts = 0;
            node.__h3sPromptInstallNextAt = 0;
            return;
        }
        const attempts = Math.min(8, (Number(node.__h3sPromptInstallAttempts) || 0) + 1);
        const delay = Math.min(2000, 120 * (2 ** Math.min(attempts - 1, 4)));
        node.__h3sPromptInstallAttempts = attempts;
        node.__h3sPromptInstallNextAt = (typeof performance !== "undefined" ? performance.now() : Date.now()) + delay;
        node.__h3sPromptInstallRetry = setTimeout(() => {
            node.__h3sPromptInstallRetry = null;
...
function installNode(nodeType, nodeData) {
    ...
    if (nodeType.prototype.__h3sEasyNodeInstalled) return;
    nodeType.prototype.__h3sEasyNodeInstalled = true;
    ...
    nodeType.prototype.onNodeCreated = function onNodeCreatedH3Easy() {
        ...
        if (resolutionWidget && !resolutionWidget.__h3sConditionalCallbackBound) {
            resolutionWidget.__h3sConditionalCallbackBound = true;
        ...
        if (advancedWidget && !advancedWidget.__h3sConditionalCallbackBound) {
            advancedWidget.__h3sConditionalCallbackBound = true;
        ...
        if (referenceMentionWidget && !referenceMentionWidget.__h3sConditionalCallbackBound) {
            referenceMentionWidget.__h3sConditionalCallbackBound = true;
    ...
    nodeType.prototype.onRemoved = function onRemovedH3Easy() {
        ...
        if (this.__h3sPromptInstallRetry) clearTimeout(this.__h3sPromptInstallRetry);
        this.__h3sPromptInstallRetry = null;
        this.__h3sPromptInstallPending = false;
        this.__h3sPromptInstallAttempts = 0;
        this.__h3sPromptInstallNextAt = 0;
        this.__h3sEditorWrap?.remove?.();
        this.__h3sEditor = null;
        this.__h3sEditorWrap = null;
        this.__h3sDomWidget = null;
...
function installLoaderNode(nodeType, nodeData) {
    ...
    if (nodeType.prototype.__h3sEasyLoaderInstalled) return;
    nodeType.prototype.__h3sEasyLoaderInstalled = true;
    const originalCreated = nodeType.prototype.onNodeCreated;
...
function installOutputNode(nodeType, nodeData) {
    ...
    if (nodeType.prototype.__h3sEasyOutputInstalled) return;
    nodeType.prototype.__h3sEasyOutputInstalled = true;
    const originalCreated = nodeType.prototype.onNodeCreated;
...
function install() {
    ...
    document.addEventListener("pointerdown", (event) => {
        for (const node of app.graph?._nodes || []) {
            const state = node?.__h3sMentionMenu;
            if (!state) continue;
            if (state.element?.contains?.(event.target) || node.__h3sEditorWrap?.contains?.(event.target)) continue;
            closeMentionMenu(node);
        }
    }, true);
    const style = document.createElement("style");
    style.textContent = `
      .h3s-prompt-editor-wrap {
      ...
      .h3s-prompt-editor:empty::before { content: attr(data-placeholder); color: var(--h3s-native-widget-muted, rgba(255,255,255,.38)); pointer-events: none; }
      .h3s-mention-chip {
        display: inline; max-width: 150px; margin: 0 1px; padding: 0; vertical-align: baseline; border: 0; border-radius: 0;
        background: transparent; color: rgba(0,226,187,.98); font-family: inherit; font-size: var(--h3s-prompt-text-size, 12px);
        font-weight: 400; line-height: inherit; letter-spacing: 0; user-select: text; cursor: text;
      }
      .h3s-mention-chip.is-unresolved { color: #ff9b9b; text-decoration: underline wavy rgba(255,110,110,.86); text-decoration-thickness: 1px; }
      .h3s-mention-chip-label { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; vertical-align: baseline; }
      .h3s-dialogue-block {
        display: inline; margin: 0 1px; padding: 2px 4px; vertical-align: 1px; border: 0; border-radius: 4px;
        background: rgba(0,226,187,.14); color: rgba(190,255,244,.98); font-family: Consolas, "Courier New", monospace; font-size: var(--h3s-prompt-text-size, 12px);
      ...
      .h3s-dialogue-block:focus { background: rgba(0,226,187,.19); box-shadow: inset 0 0 0 1px rgba(0,226,187,.26); }
      .h3s-mention-chip-thumb { display: inline-block; width: 16px; height: 16px; margin-right: 2px; object-fit: cover; border-radius: 3px; vertical-align: -2px; background: rgba(255,255,255,.12); user-select: none; }
      .h3s-mention-chip-thumb.is-image, .h3s-mention-menu-thumb.is-image { background: #5aa9f0; }
      .h3s-mention-chip-thumb.is-video, .h3s-mention-menu-thumb.is-video { position: relative; background: linear-gradient(135deg, #1557b8, #49b6ff); }
      .h3s-mention-chip-thumb.is-video::after {
      ...
      .h3s-mention-menu-thumb.is-video::after {
      ...
      .h3s-mention-menu {
        ...
        border: 1px solid var(--h3s-native-widget-outline, rgba(255,255,255,.16)); border-radius: 8px;
        background: var(--h3s-native-menu-bg, rgba(28,28,28,.98)); box-shadow: 0 16px 38px rgba(0,0,0,.42);
        color: var(--h3s-native-widget-text, rgba(255,255,255,.94)); font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
      }
      .h3s-mention-menu-title { padding: 6px 8px 7px; color: var(--h3s-native-widget-muted, rgba(255,255,255,.62)); font-size: 12px; }
      .h3s-mention-menu-empty { padding: 9px 10px; color: var(--h3s-native-widget-muted, rgba(255,255,255,.62)); font-size: 12px; }
      .h3s-mention-menu-item { display: grid; grid-template-columns: 38px minmax(0,1fr); gap: 8px; align-items: center; min-height: 42px; padding: 4px 7px; border-radius: 6px; cursor: pointer; }
      .h3s-mention-menu-item.is-active, .h3s-mention-menu-item:hover { background: rgba(160,255,178,.15); }
      .h3s-mention-menu-thumb { display: block; width: 36px; height: 36px; object-fit: cover; border-radius: 5px; background: rgba(255,255,255,.1); }
      .h3s-mention-menu-main { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; font-weight: 700; }
      .h3s-mention-menu-detail { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-top: 2px; color: var(--h3s-native-widget-muted, rgba(255,255,255,.55)); font-size: 11px; }
    `;
    document.head.append(style);
}

app.registerExtension({
    name: "H3StudioDirector",
    setup() {
        install();
    },
