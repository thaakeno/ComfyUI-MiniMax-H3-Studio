...
export function releaseSeedQueueReservation(generation) {
  ...
  return {
    ...generation,
    seed: current,
    seed_queue_reservations: Math.max(0, reservations - 1),
  };
}

export function advanceSeedAfterGeneration(generation, randomizer) {
  const current = Math.max(0, Math.trunc(Number(generation?.seed) || 0));
  if (generation?.seed_locked === true) return { ...generation, seed: current, seed_locked: true, seed_queue_reservations: 0 };
  const reservations = Math.max(0, Math.trunc(Number(generation?.seed_queue_reservations) || 0));
  if (reservations > 0) {
    // v7 reserves the next visible seed immediately after /prompt accepts the
    // current serialized prompt. The legacy execution_success path still calls
    // this function, so consume one reservation without advancing a second time.
    return { ...generation, seed: current, seed_locked: false, seed_queue_reservations: reservations - 1 };
  }
  return { ...generation, seed: nextSeedValue(current, randomizer), seed_locked: false, seed_queue_reservations: 0 };
}
